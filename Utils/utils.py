...
/***************************************************************************
 Utils
                                 A QGIS plugin
 A function library for joinTables plugin.
                              -------------------
        begin                : 2019-02-09
        git sha              : $Format:%H$
...
from qgis.core import QgsVectorLayerJoinInfo, QgsProject, QgsPalLayerSettings, \
    QgsTextFormat, QgsPropertyCollection, QgsUnitTypes, QgsVectorLayerSimpleLabeling, QFont, QColor, QgsProperty
        ...
        for k, v in g_dict.items():
            for c, e in t_dict.items():
                if g_dict[k] == t_dict[c]:
                    d[k] = d.setdefault(k, c)  # layers ids
                    f[v] = f.setdefault(v, e)  # layers names
        return d, f
